...
namespace ns3
{
	...
	bool MacRxMiddle::IsDuplicate(const WifiMacHeader *hdr, OriginatorRxStatus *originator) const
	{
	...
	Ptr<Packet> MacRxMiddle::HandleFragments(Ptr<Packet> packet, const WifiMacHeader *hdr, OriginatorRxStatus *originator)
	{
		...
		if (originator->IsDeFragmenting()) {
			if (hdr->IsMoreFragments()) {
				if (originator->IsNextFragment(hdr->GetSequenceControl())) {
					NS_LOG_DEBUG("accumulate fragment seq=" << hdr->GetSequenceNumber() << ", frag=" << hdr->GetFragmentNumber()
																									<< ", size=" << packet->GetSize());
				...
				if (originator->IsNextFragment(hdr->GetSequenceControl())) {
					NS_LOG_DEBUG("accumulate last fragment seq=" << hdr->GetSequenceNumber() << ", frag=" << hdr->GetFragmentNumber()
																											 << ", size=" << hdr->GetSize());
	...
	void MacRxMiddle::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		...
		NS_ASSERT(hdr->IsData() || hdr->IsMgt() || hdr->IsS1gBeacon() || hdr->IsTwtFrame());
