...
#define LOG_SLEEP(msg)                                                                                                                     \
	if (true)                                                                                                                                \
		NS_LOG_DEBUG("[" << (GetAID()) << "] " << msg << std::endl);
...
namespace ns3
{
	...
	TypeId StaWifiMac::GetTypeId(void)
	{
		static TypeId tid =
				TypeId("ns3::StaWifiMac")
						...
						.AddAttribute("ProbeRequestTimeout", "The interval between two consecutive probe request attempts.", TimeValue(Seconds(0.05)),
													MakeTimeAccessor(&StaWifiMac::m_probeRequestTimeout), MakeTimeChecker())
						.AddAttribute("AssocRequestTimeout", "The interval between two consecutive assoc request attempts.", TimeValue(Seconds(0.5)),
													MakeTimeAccessor(&StaWifiMac::m_assocRequestTimeout), MakeTimeChecker())
						.AddAttribute("RawDuration", "The duration of one RAW group.", TimeValue(MicroSeconds(102400)),
													MakeTimeAccessor(&StaWifiMac::GetRawDuration, &StaWifiMac::SetRawDuration), MakeTimeChecker())
						.AddAttribute("MaxTimeInQueue", "The max. time a packet stays in the DCA queue before it's dropped",
													TimeValue(MilliSeconds(10000)), MakeTimeAccessor(&StaWifiMac::m_maxTimeInQueue), MakeTimeChecker())
						.AddAttribute("MaxMissedBeacons",
													"Number of beacons which much be consecutively missed before "
													"we attempt to restart association.",
													UintegerValue(10), MakeUintegerAccessor(&StaWifiMac::m_maxMissedBeacons), MakeUintegerChecker<uint32_t>())
						.AddAttribute("StaType", "Type of station, '1' sensor, '2' offload station ", UintegerValue(1),
													MakeUintegerAccessor(&StaWifiMac::GetStaType, &StaWifiMac::SetStaType), MakeUintegerChecker<uint32_t>())
						.AddAttribute("ChannelWidth", "Channel width of the stations ", UintegerValue(0),
													MakeUintegerAccessor(&StaWifiMac::GetChannelWidth, &StaWifiMac::SetChannelWidth), MakeUintegerChecker<uint32_t>())
						.AddAttribute("ActiveProbing",
													...
													BooleanValue(false), MakeBooleanAccessor(&StaWifiMac::SetActiveProbing, &StaWifiMac::GetActiveProbing),
													MakeBooleanChecker())
						.AddTraceSource("Assoc", "Associated with an access point.", MakeTraceSourceAccessor(&StaWifiMac::m_assocLogger),
														"ns3::Mac48Address::TracedCallback")
						.AddTraceSource("DeAssoc", "Association with an access point lost.", MakeTraceSourceAccessor(&StaWifiMac::m_deAssocLogger),
														"ns3::Mac48Address::TracedCallback")
						.AddTraceSource("S1gBeaconMissed", "Fired when a beacon is missed.", MakeTraceSourceAccessor(&StaWifiMac::m_beaconMissed),
														"ns3::StaWifiMac::S1gBeaconMissedCallback")

						.AddTraceSource("NrOfTransmissionsDuringRAWSlot", "Nr of transmissions during RAW slot",
														MakeTraceSourceAccessor(&StaWifiMac::nrOfTransmissionsDuringRAWSlot), "ns3::TracedValueCallback::Uint16");
		return tid;
	}

	StaWifiMac::StaWifiMac()
			: m_state(BEACON_MISSED), m_probeRequestEvent(), m_assocRequestEvent(), m_disassocRequestEvent(),
				m_beaconWatchdogEnd(Seconds(0.0)), m_hasAnyAccessPointTwtAgreements{false}
	{
		...
		m_fastAssocType = false;	// centraied control
		m_fastAssocThreshold = 0; // allow some station to associate at the begining
		Ptr<UniformRandomVariable> m_rv = CreateObject<UniformRandomVariable>();
		m_assocValue = m_rv->GetValue(0, 999);

		m_firstBeacon = true;
		m_receivingBeacon = false;
		m_timeDifferenceBeacon = 0;
		m_timeBeacon = 0;
		m_outsideRaw = false;
		m_stationRawSlot = false;
		m_waitingAck = false;
	...
	void StaWifiMac::SendAnnouncedTwtWakeupMessage(Mac48Address to)
	{
		SendPspoll(to);
	}
	void StaWifiMac::ConfigureTwt()
	{
		NS_LOG_UNCOND("Configure TWT called (empty fn for now), seems like we can do everything in regular wifi mac.");
	}
	...
	uint32_t StaWifiMac::GetStaType(void) const
	{
		return m_staType;
	}

	void StaWifiMac::SetStaType(uint32_t statype)
	{
		m_staType = statype;
	}

	void StaWifiMac::SetChannelWidth(uint32_t width)
	{
		m_channelWidth = width;
	}

	uint32_t StaWifiMac::GetChannelWidth(void) const
	{
		NS_LOG_UNCOND(GetAddress() << ", GetChannelWidth " << m_channelWidth);
		return m_channelWidth;
	}

	uint32_t StaWifiMac::GetAID(void) const
	{
		NS_ASSERT(((1 <= m_aid) && (m_aid <= 8191)) || (m_aid == 8192));
		return m_aid;
	}

	Time StaWifiMac::GetRawDuration(void) const
	{
		NS_LOG_FUNCTION(this);
		return m_rawDuration;
	}

	bool StaWifiMac::Is(uint8_t blockbitmap, uint8_t j)
	{
		return (((blockbitmap >> j) & 0x01) == 0x01);
	}

	void StaWifiMac::SetAID(uint32_t aid)
	{
		NS_ASSERT((1 <= aid) && (aid <= 8191));
		m_aid = aid;
	}

	void StaWifiMac::SetRawDuration(Time interval)
	{
		NS_LOG_FUNCTION(this << interval);
		m_rawDuration = interval;
	}

	void StaWifiMac::SetDataBuffered()
	{
		m_dataBuffered = true;
	}

	void StaWifiMac::ClearDataBuffered()
	{
		m_dataBuffered = false;
	}

	void StaWifiMac::SetInRAWgroup()
	{
		m_inRawGroup = true;
	}

	void StaWifiMac::UnsetInRAWgroup()
	{
		m_inRawGroup = false;
	}
	...
	void StaWifiMac::SendPspoll(void)
	{
		SendPspoll(GetBssid());
	}

	void StaWifiMac::SendPspoll(Mac48Address to)
	{
		NS_LOG_FUNCTION(this);
		WifiMacHeader hdr;
	...
	void StaWifiMac::SendPspollIfnecessary(void)
	{
		...
		} else if (!m_rawStart && m_dataBuffered &&
							 !m_outsideRawEvent.IsRunning()) // in case the next beacon coming during RAW, could it happen?
		{
			// SendPspoll ();
		}
	}
	...
	void StaWifiMac::S1gTIMReceived(S1gBeaconHeader beacon)
	{
		...
		if (m_TIM.GetDTIMCount() == 0) {
			...
			m_pageSlice = beacon.GetpageSlice();
			// TIM element length is 2 when both Partial Virtual Bitmap and Bitmap Control fields are 0 - they are not present
			// in TIM TIM element length is 3 when Partial Virtual Bitmap is 0 - it is not present in TIM else, TIM element
			// length is 3 + size of Encoded Blocks in Partial Virtual Bitmap
			...
			if (m_selfPage != m_pageSlice.GetPageindex()) {
				NS_LOG_DEBUG("[aid=" << GetAID() << "] is not located in page " << (int)m_Pageindex << ", but in page " << (int)m_selfPage
														 << ". Scheduling sleep until the next DTIM beacon.");
				m_pagedInDtim = false;
			...
			if (m_selfBlock < m_BlockOffset) {
				...
				NS_LOG_DEBUG("[aid=" << GetAID() << "] belongs to a block " << (int)m_selfBlock << " which is not included in current page ("
														 << (int)m_selfPage << ") because block offset in Page Slice element is " << (int)m_BlockOffset
														 << " Scheduling sleep until the next DTIM beacon.");
				m_pagedInDtim = false;
			...
			if (selfPageSliceNumber != m_TIM.GetPageSliceNum()) {
				...
				NS_LOG_DEBUG("[AID: " << GetAID() << "]: This page slice (" << (int)m_TIM.GetPageSliceNum()
															<< ") is not my page slice. My page slice number is " << selfPageSliceNumber);
				// sleep until my TIM
		...
		/*if ((m_DTIMCount == 0 && m_TIMOffset ==0) || (m_DTIMCount !=0 && (m_DTIMPeriod - m_DTIMCount) == m_TIMOffset) )
		 ...
		 //wake up after the last page slice, this may happen before the next DTIM, depends on TIM offset and page slice
		 count.
		 }
		...
		do {
			...
			if (blockind == m_selfBlock) {
				if ((blockbitmap & (1 << m_selfSubBlock)) == 0) // no packets in subblock
				{
					...
					{
						NS_LOG_INFO("[aid=" << this->GetAID() << "]"
																<< "Downlink packet indicated for me.");
						// reset m_pagedInDtim;
						m_pagedInDtim = false;
	...
	void StaWifiMac::GoToSleepNextTIM(S1gBeaconHeader beacon) // to do, merge with GoToSleepCurrentTIM
	{
		...
		Time timeUntilNextTim = MicroSeconds(beacon.GetBeaconCompatibility().GetBeaconInterval()) -
														MicroSeconds(Simulator::Now().GetMicroSeconds() % beacon.GetBeaconCompatibility().GetBeaconInterval());
		...
		// std::cout << "+++At " << Simulator::Now().GetMicroSeconds() << "us: GoToSleepNextTIM:" <<
		// sleepTime.GetMicroSeconds() << " FULL" << std::endl; std::cout << "At " <<
		// MicroSeconds(Simulator::Now().GetMicroSeconds() % beacon.GetBeaconCompatibility().GetBeaconInterval()) << " us
		// OFFSET" << std::endl; std::cout << "-*-*-*-*-*-*-*-*-TIME UNTIL NEXT TIM=" << timeUntilNextTim.GetMicroSeconds()
		// << "us" << " --NOW: " << Simulator::Now().GetMicroSeconds() << std::endl; std::cout << "-*-*-*-*-*-*-*-*-SLEEP
		// TIME=" << sleepTime.GetMicroSeconds() << "us" << std::endl;
	...
	void StaWifiMac::GoToSleepCurrentTIM(S1gBeaconHeader beacon)
	{
		...
		// std::cout << "BEACON Interval=" << (int)beacon.GetBeaconCompatibility().GetBeaconInterval() << std::endl;
		Time sleepTime = MicroSeconds(beacon.GetBeaconCompatibility().GetBeaconInterval() * BeaconNumForTIM);
		// std::cout << "+++At " << Simulator::Now().GetMicroSeconds() << "us: GoToSleepCurrentTIM:" <<
		// sleepTime.GetMicroSeconds() << " FULL" << std::endl;
		...
		Time timeUntilNextTim = MicroSeconds(beacon.GetBeaconCompatibility().GetBeaconInterval()) -
														MicroSeconds(Simulator::Now().GetMicroSeconds() % beacon.GetBeaconCompatibility().GetBeaconInterval());
		// std::cout << "At " << MicroSeconds(Simulator::Now().GetMicroSeconds() %
		// beacon.GetBeaconCompatibility().GetBeaconInterval()) << " us OFFSET" << std::endl; std::cout << "TIME UNTIL NEXT
		// TIM=" << timeUntilNextTim.GetMicroSeconds() << "us" << std::endl;
		GoToSleep(sleepTime);
	}
	...
	void StaWifiMac::GoToSleep(Time sleeptime)
	{
		...
		if (sleeptime < GetEarlyWakeTime()) {
			NS_LOG_DEBUG("At " << Simulator::Now().GetSeconds() << " s AID " << this->GetAID() << " CANNOT SLEEP because sleeptime "
												 << sleeptime.GetSeconds() << " s < EarlyWakeTime.");
			return;
		}
		sleeptime -= GetEarlyWakeTime();
		if (!m_low->GetPhy()->IsStateSleep() && (sleeptime.GetMicroSeconds() > 0)) {
			NS_LOG_DEBUG("At " << Simulator::Now().GetSeconds() << " s AID " << this->GetAID() << " switches to SLEEP. Schedule wake-up after "
												 << sleeptime.GetMicroSeconds() << " us.");
			m_low->GetPhy()->SetSleepMode();
			Simulator::Schedule(sleeptime, &StaWifiMac::WakeUp, this);
			// std::cout << "+++At " << Simulator::Now().GetMicroSeconds() << "us: GoToSleep:" << sleeptime.GetMicroSeconds()
			// << " FULL" << std::endl;
		}
	}
	...
	void StaWifiMac::GoToSleepBinary(int value)
	{
		if (IsAssociated() && !m_receivingBeacon) {
			if (value == 0) {
				...
				} else if (m_waitingAck && (m_outsideRaw || m_stationRawSlot)) {
	...
	void StaWifiMac::SleepIfQueueIsEmpty(bool value)
	{
		m_waitingAck = !value;

		if (IsAssociated() && !m_receivingBeacon) {
			if (!HasPacketsInQueue() && !m_waitingAck)

			{
				m_low->GetPhy()->SetSleepMode();
			}

			if (HasPacketsInQueue() && !m_waitingAck && !m_outsideRaw && !m_stationRawSlot)
			...
			if (!m_outsideRaw && !m_stationRawSlot) {
				m_low->GetPhy()->SetSleepMode();
			}

			if (m_waitingAck && (m_outsideRaw || m_stationRawSlot))
	...
	bool StaWifiMac::HasPacketsInQueue()
	{
		...
		return (m_edca.find(AC_VO)->second->GetEdcaQueue()->GetSize() > 0 || m_edca.find(AC_VI)->second->GetEdcaQueue()->GetSize() > 0 ||
						m_edca.find(AC_BE)->second->GetEdcaQueue()->GetSize() > 0 || m_edca.find(AC_BK)->second->GetEdcaQueue()->GetSize() > 0);
	}

	// actions to do when station wakes up for beacon
	void StaWifiMac::BeaconWakeUp(void)
	{
		...
		m_beaconWakeUpEvent = Simulator::Schedule(m_beaconInterval, &StaWifiMac::BeaconWakeUp, this);
		m_receivingBeacon = true;
		// NS_LOG_UNCOND ( GetAddress () << ",Wake Up for beacon," << Simulator::Now().GetSeconds() << ",beacon interval,"
		// << beaconInterval.GetSeconds()); NS_LOG_UNCOND ( GetAddress () << ",Wake Up for beacon," <<
		// Simulator::Now().GetSeconds());
	}

	void StaWifiMac::OnAssociated()
	{
	...
	void StaWifiMac::OnDeassociated()
	{
	...
	void StaWifiMac::RawSlotStartBackoff(void)
	{
		...
		// NS_LOG_UNCOND ( m_low->GetAddress () << " Inside backoff scheduled " << Simulator::Now() <<
		// m_currentslotDuration);
	...
	void StaWifiMac::RawSlotStartBackoffPostpone(void)
	{
		m_pspollDca->AccessAllowedIfRaw(true);
		m_dca->AccessAllowedIfRaw(true);
		m_edca.find(AC_VO)->second->AccessAllowedIfRaw(true);
	...
	void StaWifiMac::InsideBackoff(void)
	{
	...
	void StaWifiMac::StartRawbackoff(void)
	{
		m_pspollDca->RawStart(m_slotDuration,
													m_crossSlotBoundaryAllowed); // not really start raw useless allowedAccessRaw is true;
	...
	void StaWifiMac::OutsideRawStartBackoff(void)
	{
		if (m_insideBackoffEvent.IsRunning()) {
			m_insideBackoffEvent.Cancel();
		}
...
*/
	...
	void StaWifiMac::SetWifiRemoteStationManager(Ptr<WifiRemoteStationManager> stationManager)
	{
	...
	void StaWifiMac::SendProbeRequest(void)
	{
		...
		m_probeRequestEvent = Simulator::Schedule(m_probeRequestTimeout, &StaWifiMac::ProbeRequestTimeout, this);
	}

	void StaWifiMac::TxOk(const WifiMacHeader &hdr)
	{
		NS_LOG_FUNCTION(this);
		RegularWifiMac::TxOk(hdr);

		if (hdr.IsDisassociation() && IsWaitDisAssocTxOk()) {
	...
	void StaWifiMac::SendDisAssociationRequest(void)
	{
		NS_LOG_FUNCTION(this);
		WifiMacHeader hdr;
		hdr.SetDisAssocReq();
		...
		m_disassocRequestEvent = Simulator::Schedule(m_assocRequestTimeout, &StaWifiMac::SendDisAssociationRequest, this);
	...
	void StaWifiMac::SendAssociationRequest(void)
	{
		...
		if (!m_s1gSupported) {
			m_fastAssocThreshold = 1023;
		}

		if (m_assocValue < m_fastAssocThreshold) {
			WifiMacHeader hdr;
			hdr.SetAssocReq();
			hdr.SetAddr1(GetBssid());
			hdr.SetAddr2(GetAddress());
		...
		m_assocRequestEvent = Simulator::Schedule(m_assocRequestTimeout, &StaWifiMac::AssocRequestTimeout, this);
	}
	...
	void StaWifiMac::RestartBeaconWatchdog(Time delay, bool forceValue)
	{
		NS_LOG_FUNCTION(this << delay);
		if (forceValue) {
			m_beaconWatchdogEnd = Simulator::Now() + delay;
		} else {
	...
	bool StaWifiMac::IsWaitDisAssocTxOk(void) const
	{
	...
	void StaWifiMac::Enqueue(Ptr<const Packet> packet, Mac48Address to)
	{
		...
		// code RAW
		if (m_low->GetPhy()->IsStateSleep() && m_stationRawSlot) {
			NS_LOG_DEBUG(m_low->GetAddress() << ",enqueue in stationrawslot," << Simulator::Now().GetSeconds());
			WakeUp();
		}
		if (m_low->GetPhy()->IsStateSleep() && m_outsideRaw) {
	...
	void StaWifiMac::S1gBeaconReceived(S1gBeaconHeader beacon)
	{
		...
		if (m_outsideRawEvent.IsRunning()) {
			m_outsideRawEvent.Cancel(); // avoid error when actual beacon interval become shorter, otherwise, AccessAllowedIfRaw will set
																	// again after raw starting Simulator::ScheduleNow(&StaWifiMac::OutsideRawStartBackoff, this);
		}
		...
		if (m_aid == 8192) // send assoication request when Staion is not assoicated
		{
			m_dca->AccessAllowedIfRaw(true);
							 ...
							 m_dataBuffered) // if m_pagedStaRaw is true, only m_dataBuffered can access channel
		{
	...
	void StaWifiMac::HandlePacketDrop(Ptr<Packet> packet)
	{
		NotifyRxDrop(packet);
	}
	void StaWifiMac::HandleDataPacket(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		if (!IsAssociated()) {
			NS_LOG_LOGIC("Received data frame while not associated: ignore");
			HandlePacketDrop(packet);
			return;
		}
	...
	void StaWifiMac::HandleBeacon(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
	...
	void StaWifiMac::HandleGoodS1gBeacon(Ptr<Packet> packet, const WifiMacHeader *hdr, S1gBeaconHeader &beacon)
	{
		m_timeDifferenceBeacon = Simulator::Now().GetMicroSeconds() - m_timeBeacon;
		m_timeBeacon = Simulator::Now().GetMicroSeconds();
		if (m_firstBeacon) {
			m_beaconInterval = MicroSeconds(beacon.GetBeaconCompatibility().GetBeaconInterval());
			int64x64_t interval = static_cast<int64x64_t>(m_beaconInterval);
			interval = interval / 10000000;
			interval = (interval).GetHigh();
			interval = interval * 10000000;
			Time intervalFirstBeacon = static_cast<Time>(interval);
			// std::cout << "++++++++++++++++++us beaconInterval = " << beaconInterval << "; Now=" <<
			// Simulator::Now().GetMicroSeconds() << std::endl; Time intervallobeacon = MicroSeconds (98920);
		...
		for (uint8_t raw_index = 0; raw_index < RAW_number; raw_index++) {
			...
			if (ass.GetRawGroupPage() == ((GetAID() >> 11) & 0x0003)) // in the page indexed
			{
				...
				if ((ass.GetRawGroupAIDStart() <= (GetAID() & 0x07ff)) && ((GetAID() & 0x07ff) <= ass.GetRawGroupAIDEnd())) {
					...
					// NS_LOG_UNCOND (Simulator::Now () << ", StaWifiMac:: GetAID() = " << GetAID() <<  ", m_statSlotStart=" <<
					// m_statSlotStart << ", m_lastRawDurationus = " << m_lastRawDurationus << ", m_currentslotDuration = " <<
					// m_currentslotDuration);
					// break; //break should not used if multiple RAW is supported
				}
				// NS_LOG_UNCOND (Simulator::Now () << ", StaWifiMac:: GetAID() = " << GetAID() << ", raw_start =" << raw_start
				// << ", raw_end=" << raw_end << ", m_statSlotStart=" << m_statSlotStart << ", m_lastRawDurationus = " <<
				// m_lastRawDurationus << ", m_currentslotDuration = " << m_currentslotDuration);
			}
		}
		m_rawStart = true; //?
		...
		m_fastAssocType = AuthenCtrl.GetControlType();
		if (!m_fastAssocType) // only support centralized cnotrol
		{
			m_fastAssocThreshold = AuthenCtrl.GetThreshold();
		}
	}
	void StaWifiMac::HandleS1gBeacon(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		// NS_LOG_UNCOND("Received a S1G beacon.");
		...
		if (goodBeacon) {
			HandleGoodS1gBeacon(packet, hdr, beacon);
		}
	...
	void StaWifiMac::HandleProbeResponse(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
	...
	void StaWifiMac::HandleAssociationResponse(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
	...
	void StaWifiMac::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		...
		// TODO: Verify commenting this assert is OK
		// Is also present in ns3 main. Unsure why.
		// NS_ASSERT(!hdr->IsCtl());
		// TODO: Verify commenting this if is OK.
		// Was also present in main branch ns-3. Unsure why.
		...
		// If address is addressed to another address than ours, that isn't a group, drop packet because it's not intended
		// for us
		if (hdr->GetAddr1() != GetAddress() && !hdr->GetAddr1().IsGroup()) {
			NS_LOG_LOGIC("packet is not for us");
			HandlePacketDrop(packet);
		}
		// If we get an association request or a probe request, drop it. We're not an AP.
		...
		// Handle beacons
			...
			HandleBeacon(packet, hdr);
		}
		// Handle data packets
		else if (hdr->IsData()) {
			HandleDataPacket(packet, hdr);
		}
		// handle S1g beacons
		else if (hdr->IsS1gBeacon()) {
			HandleS1gBeacon(packet, hdr);
		} else if (hdr->IsProbeResp()) {
			HandleProbeResponse(packet, hdr);
		} else if (hdr->IsAssocResp()) {
			HandleAssociationResponse(packet, hdr);
		} else {
			// Invoke the receive handler of our parent class to deal with any
			// other frames. Specifically, this will handle Block Ack-related
			// Management Action frames and TWT related frames.
	...
	S1gCapabilities StaWifiMac::GetS1gCapabilities(void) const
	{
	...
	void StaWifiMac::HandleTwtChanges()
	{
		// This function is used as a callback when anything about the TWT configuration changes.
		// We need to be able to access the MAC address of the AP we're associated with, somehow?

		// Put address in variable so we can safely address-of it

		auto address = GetBssid();
		// And use the address to get the correct agreements.
		auto apAgreements = GetTwtAgreements(&address);
		if (apAgreements.empty()) {
			if (m_hasAnyAccessPointTwtAgreements) {
				// We used to have a TWT session with the AP, that is now expired
				// So we must go back to the default interval for beacon watch dog.
				RestartBeaconWatchdog(m_beaconInterval * m_maxMissedBeacons, true);
				// Record that we don't have any access point agreements, so we don't keep resetting.
				m_hasAnyAccessPointTwtAgreements = false;
			} // Else no changes, so nothing for us to do.
		} else {
			auto closest = Time::Max();
			for (auto agreement : apAgreements) {
				if (agreement->GetTargetWakeTime() < closest) {
					closest = agreement->GetTargetWakeTime();
				}
			}
			// Transform closest time into the delay expected by beacon watchdog.
			closest -= Simulator::Now();
			// Then set the new beacon watch dog delay. We allow up to the Target Wake Time without penalty,
			// Then from there we're allowed to miss the standard amount of beacons again.
			RestartBeaconWatchdog(closest + m_beaconInterval * m_maxMissedBeacons);
		}
		NS_LOG_UNCOND("TWT agreements changed for " << GetAddress() << ".");
	}
} // namespace ns3
