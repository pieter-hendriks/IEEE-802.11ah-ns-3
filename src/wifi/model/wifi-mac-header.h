...
namespace ns3
{
	...
	public:
		...
		/**
		 * Set Type/Subtype values for a TWT frame
		 */
		void SetTwtFrame(void);
		/**
		 * Set Type/Subtype values for a TACK frame
		 */
		void SetTackFrame(void);
		/**
		...
		/**
		 * Functions to set the values for the TACK frame.
		 */
		void SetTackBeaconSequence(uint8_t sequence);
		void SetTackTimestamp(Time time);
		void SetTackNextTwtInfo(Time time, uint8_t flowid);
		/**
		...
		/**
		 * Return true if the header is a TWT frame header.
		 *
		 * \return true if the header is a TWT frame header, false otherwise
		 */
		bool IsTwtFrame(void) const;
		/**
		 * Return true if the header is a TACK frame header.
		 *
		 * \return true if the header is a TACK frame header, false otherwise
		 */
		bool IsTackFrame(void) const;
		/**
		...
		bool IsNextTwtFieldPresent() const;
		std::pair<uint8_t, uint64_t> GetTackNextTwtInfo() const;
		// Control functions for TWT ctrl options
		// Documentation/Field value meanings lacking so far - but is mostly in notes.md
		// Anything missing needs to be looked up in standard - section references at least should be in notes.md
		void SetBandwidth(uint8_t i);
		void SetDynamicBandwidth(uint8_t i);
		void SetProtectedFrame(uint8_t i);
		void SetHTControlFlag(uint8_t i);
		void SetHTControl(uint32_t i);
		uint8_t GetBandwidth() const;
		uint8_t GetDynamicBandwidth() const;
		uint8_t GetProtectedFrame() const;
		uint8_t GetHTControlFlag() const;
		uint32_t GetHTControl() const;
	...
	private:
		 ...
		 */
		void SetFrameControl(uint16_t control); // for test
		/**
		...
		uint8_t m_ctrlWep;
		uint8_t m_ctrlOrder;
		uint8_t m_ctrlBandwidth;
		uint8_t m_ctrlDynamicBandwidth;
		uint8_t m_ctrlProtectedFrame;
		uint8_t m_ctrlHtControlFlag;
		uint32_t m_ctrlHtControl;
		uint8_t m_ctrlTackBeaconSeq;
		uint64_t m_ctrlTackTimestamp;
		uint64_t m_ctrlTackNextTwt;
		uint8_t m_ctrlTackNextTwtInfoPresent;
		uint8_t m_ctrlTackFlowIdentifier;
