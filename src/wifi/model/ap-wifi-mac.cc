...
#define LOG_TRAFFIC(msg)                                                                                                                   \
	if (true)                                                                                                                                \
		NS_LOG_DEBUG(Simulator::Now().GetMicroSeconds() << " " << msg << std::endl);
	...
	TypeId ApWifiMac::GetTypeId(void)
	{
		static TypeId tid =
				TypeId("ns3::ApWifiMac")
						...
						.AddAttribute("BeaconInterval", "Delay between two beacons", TimeValue(MicroSeconds(102400)),
													MakeTimeAccessor(&ApWifiMac::GetBeaconInterval, &ApWifiMac::SetBeaconInterval), MakeTimeChecker())
						.AddAttribute("BeaconJitter",
													...
													StringValue("ns3::UniformRandomVariable"), MakePointerAccessor(&ApWifiMac::m_beaconJitter),
													MakePointerChecker<UniformRandomVariable>())
						.AddAttribute("Outputpath", "output path to store info of sensors and offload stations", StringValue("stationfile"),
													MakeStringAccessor(&ApWifiMac::m_outputpath), MakeStringChecker())
						.AddAttribute("EnableBeaconJitter", "If beacons are enabled, whether to jitter the initial send event.", BooleanValue(false),
													MakeBooleanAccessor(&ApWifiMac::m_enableBeaconJitter), MakeBooleanChecker())
						.AddAttribute("BeaconGeneration", "Whether or not beacons are generated.", BooleanValue(true),
													MakeBooleanAccessor(&ApWifiMac::SetBeaconGeneration, &ApWifiMac::GetBeaconGeneration), MakeBooleanChecker())
						.AddAttribute("ChannelWidth", "Channel width of the stations ", UintegerValue(0),
													MakeUintegerAccessor(&ApWifiMac::GetChannelWidth, &ApWifiMac::SetChannelWidth), MakeUintegerChecker<uint32_t>())
						.AddAttribute("NRawGroupStas", "Number of stations in one Raw Group", UintegerValue(6000),
													MakeUintegerAccessor(&ApWifiMac::GetRawGroupInterval, &ApWifiMac::SetRawGroupInterval),
													MakeUintegerChecker<uint32_t>())
						.AddAttribute("NRawStations", "Number of total stations support RAW", UintegerValue(100),
													MakeUintegerAccessor(&ApWifiMac::GetTotalStaNum, &ApWifiMac::SetTotalStaNum), MakeUintegerChecker<uint32_t>())
						.AddAttribute("SlotFormat", "Slot format", UintegerValue(1),
													MakeUintegerAccessor(&ApWifiMac::GetSlotFormat, &ApWifiMac::SetSlotFormat), MakeUintegerChecker<uint32_t>())
						.AddAttribute("SlotCrossBoundary", "cross slot boundary or not", UintegerValue(1),
													MakeUintegerAccessor(&ApWifiMac::GetSlotCrossBoundary, &ApWifiMac::SetSlotCrossBoundary),
													MakeUintegerChecker<uint32_t>())
						.AddAttribute("SlotDurationCount", "slot duration count", UintegerValue(1000),
													MakeUintegerAccessor(&ApWifiMac::GetSlotDurationCount, &ApWifiMac::SetSlotDurationCount),
													MakeUintegerChecker<uint32_t>())
						.AddAttribute("SlotNum", "Number of slot", UintegerValue(2),
													MakeUintegerAccessor(&ApWifiMac::GetSlotNum, &ApWifiMac::SetSlotNum), MakeUintegerChecker<uint32_t>())
						.AddAttribute("RPSsetup", "configuration of RAW", RPSVectorValue(), MakeRPSVectorAccessor(&ApWifiMac::m_rpsset),
													MakeRPSVectorChecker())
						.AddTraceSource("S1gBeaconBroadcasted", "Fired when a beacon is transmitted",
														MakeTraceSourceAccessor(&ApWifiMac::m_transmitBeaconTrace), "ns3::ApWifiMac::S1gBeaconTracedCallback")
						.AddTraceSource("RpsIndex", "Fired when RPS index changes", MakeTraceSourceAccessor(&ApWifiMac::m_rpsIndexTrace),
														"ns3::TracedValueCallback::Uint16")
						.AddTraceSource("RawGroup", "Fired when RAW group index changes", MakeTraceSourceAccessor(&ApWifiMac::m_rawGroupTrace),
														"ns3::TracedValueCallback::Uint8")
						.AddTraceSource("RawSlot", "Fired when RAW slot index changes", MakeTraceSourceAccessor(&ApWifiMac::m_rawSlotTrace),
														"ns3::TracedValueCallback::Uint8")
...
"ns3::S1gApWifiMac::RawSlotStartedCallback")*/
						.AddTraceSource("PacketToTransmitReceivedFromUpperLayer", "Fired when packet is received from the upper layer",
														MakeTraceSourceAccessor(&ApWifiMac::m_packetToTransmitReceivedFromUpperLayer),
														"ns3::S1gApWifiMac::PacketToTransmitReceivedFromUpperLayerCallback")
						.AddAttribute("PageSliceSet", "configuration of PageSlice", pageSliceValue(), MakepageSliceAccessor(&ApWifiMac::m_pageslice),
													MakepageSliceChecker())
						.AddAttribute("PageSlicingActivated", "Whether or not page slicing is activated.", BooleanValue(true),
													MakeBooleanAccessor(&ApWifiMac::SetPageSlicingActivated, &ApWifiMac::GetPageSlicingActivated),
													MakeBooleanChecker())
						.AddAttribute("TIMSet", "configuration of TIM", TIMValue(), MakeTIMAccessor(&ApWifiMac::m_TIM), MakeTIMChecker());
		/*
	...
	if (aid == -1)
			NS_LOG_INFO (Simulator::Now().GetMicroSeconds() << " ms: AP cannot forward down data because There is no RAW for
	this MAC address.");
	*/
			do {
				aid++;
			...
			uint8_t toTim = 0;
			//= (block - m_pageslice.GetBlockOffset()) % m_pageslice.GetPageSliceLen(); //TODO make config alignment between
			// TIM and RAW e.g. if AID belongs to TIM0 it cannot belong to RAW located in TIM3
			...
			NS_LOG_DEBUG("At " << Simulator::Now().GetSeconds() << " s AP scheduling transmission for [aid=" << aid << "] "
												 << wait.GetMicroSeconds() << " us from now, at " << Simulator::Now().GetSeconds() + wait.GetSeconds() << ".");
			/*
	...
	Time ApWifiMac::GetSlotStartTimeFromAid(uint16_t aid) const
	{
		...
		NS_ASSERT(block >= m_pageslice.GetBlockOffset());
		uint8_t toTim = 0;
		//= (block - m_pageslice.GetBlockOffset()) % m_pageslice.GetPageSliceLen(); //TODO make config alignment between TIM
		// and RAW e.g. if AID belongs to TIM0 it cannot belong to RAW located in TIM3
		...
		for (uint8_t raw_index = 0; raw_index < RAW_number; raw_index++) {
			...
			if (ass.GetRawGroupAIDStart() <= aid && aid <= ass.GetRawGroupAIDEnd()) {
				...
				NS_LOG_DEBUG("[aid=" << aid << "] is located in RAW " << (int)raw_index + 1 << " in slot " << statRawSlot + 1
														 << ". RAW slot start time relative to the beacon = " << start.GetMicroSeconds() << " us.");
				x = 1;
				return start;
			}
		}
		...
		/*currentRAW_start += (500 + slotDurationCount * 120) * slotNum;
NS_LOG_DEBUG ("[aid=" << aid << "] is located outside all RAWs. It can start contending " << currentRAW_start << " us
after the beacon.");*/
		NS_ASSERT(x);
	...
	SupportedRates ApWifiMac::GetSupportedRates(void) const
	{
		...
		// Also the standard mentioned that at leat 1 element should be included in the SupportedRates the rest can be in
		// the ExtendedSupportedRates
		if (m_htSupported) {
		...
		for (uint32_t i = 0; i < m_phy->GetNModes(); i++) {
			...
			if (mode.IsMandatory() && ((mode.GetModulationClass() != WIFI_MOD_CLASS_DSSS) || mode == WifiPhy::GetDsssRate1Mbps() ||
		...
		for (uint16_t i = 0; i <= 7; i++) // 8 subblock in each block.
		{
			...
			for (uint16_t j = 0; j <= 7; j++) // 8 stations in each subblock
			{
				...
				if (m_stationManager->IsAssociated(m_AidToMacAddr.find(sta_aid)->second) &&
						HasPacketsInQueueTo(m_AidToMacAddr.find(sta_aid)->second)) {
					blockBitmap = blockBitmap | (1 << i);
					NS_LOG_DEBUG("[aid=" << sta_aid << "] "
															 << "paged");
					// if there is at least one station associated with AP that has FALSE for PageSlicingImplemented within this
					// page then m_PageSliceNum = 31
	...
	uint8_t ApWifiMac::HasPacketsToSubBlock(uint16_t subblockInd, uint16_t blockInd, uint16_t PageInd)
	{
	...
	void ApWifiMac::SetaccessList(std::map<Mac48Address, bool> list)
	{
		...
		for (uint32_t k = 1; k <= m_totalStaNum; k++) {
			...
			// NS_LOG_UNCOND ( "aid "  << k << ", send " << list.find(stasAddr)->second << ", at " << Simulator::Now () << ",
			// size " << list.size ());
		}
	...
	void ApWifiMac::SendOneBeacon(void)
	{
		...
		if (m_s1gSupported) {
			...
			if (m_DTIMCount == 0 && GetPageSlicingActivated()) // TODO filter when GetPageSlicingActivated() is false
			{
				...
				if (m_pagebitmap) // for now, only configure Page Bit map based on real-time traffic, other parameters
			...
			m_rps = m_S1gRawCtr.UpdateRAWGroupping (m_sensorList, m_OffloadList, m_receivedAid,
			m_beaconInterval.GetMicroSeconds (), m_outputpath); m_receivedAid.clear (); //release storage
			...
			if (m_pageslice.GetPageSliceCount() == 0) {
				m_PageSliceNum = 31;
			} else if (m_PageSliceNum != 31 && m_pageslice.GetPageSliceCount() != 0) {
				...
				NS_ASSERT(m_PageSliceNum <
									m_pageslice.GetPageSliceCount()); // or do not use m_PageSliceNum when it's larger (equal) than slice count.
			}
			...
			/*if (m_pageslice.GetPageSliceCount() == 0 && numPagedStas > 0)// special case
{
	...
	{
			// 32nd TIM in this DTIM can contain DL information for (1) STAs that do not support page slicing or (2)STAs that
support page slicing and
...
else if (m_pageslice.GetPageSliceCount () == 0 && numPagedStas == 0)
{
			...
			} else if (m_pageslice.GetPageBitmapLength() > 0) // Last Page slice has max value of 31 (5 bits)
			{
				...
				if (m_pageslice.GetPageSliceCount() > 0)
					...
					NumEncodedBlock = 31;
				// As page bitmap of page slice element is fixed to 4 bytes for now, "m_pageslice.GetInformationFieldSize ()" is
				// alwawys 8. Section 9.4.2.193 oage slice element, Draft 802.11ah_D9.0
	...
	*/
			...
			// NS_ASSERT (m_DTIMPeriod - m_DTIMCount + m_DTIMOffset == m_DTIMPeriod || (m_DTIMCount == 0 && m_DTIMOffset ==
			// 0));
			...
			m_lastBeaconSequence = beacon.GetChangeSequence();
			...
			for (uint32_t g = 0; g < nRaw; g++) {
				...
				for (uint32_t i = 0; i < m_rps->GetRawAssigmentObj(g).GetSlotNum(); i++) {
					...
					Simulator::Schedule(bufferTimeToAllowBeaconToBeReceived + timeToSlotStart, &ApWifiMac::SetaccessList, this, m_accessList);

					Simulator::Schedule(bufferTimeToAllowBeaconToBeReceived + timeToSlotStart, &ApWifiMac::OnRAWSlotStart, this, RpsIndex, g + 1,
															i + 1);
			...
			// NS_LOG_UNCOND(GetAddress () << ", " << startaid << "\t" << endaid << ", at " << Simulator::Now () << ",
			// bufferTimeToAllowBeaconToBeReceived " << bufferTimeToAllowBeaconToBeReceived);
		} else {
	...
	void ApWifiMac::OnRAWSlotStart(uint16_t rps, uint8_t rawGroup, uint8_t slot)
	{
		...
		// NS_LOG_UNCOND("AP RAW SLOT START FOR RAW GROUP " << std::to_string(rawGroup) << " SLOT " <<
		// std::to_string(slot));
	}
	...
	void ApWifiMac::HandleDataPacket(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
	...
	void ApWifiMac::HandleProbeRequest(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		// Add unused parameter for uniformity
		NS_UNUSED(packet);
		NS_ASSERT(hdr->GetAddr1().IsBroadcast());
		SendProbeResp(hdr->GetAddr2());
	}
	void ApWifiMac::HandleAssociationRequest(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		Mac48Address from = hdr->GetAddr2();
	...
	void ApWifiMac::HandleDisassociation(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		Mac48Address from = hdr->GetAddr2();
	...
	void ApWifiMac::HandleManagementPacket(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		if (hdr->IsProbeReq()) {
			HandleProbeRequest(packet, hdr);
		} else if (hdr->GetAddr1() == GetAddress()) {
			if (hdr->IsAssocReq()) {
				HandleAssociationRequest(packet, hdr);
			} else if (hdr->IsDisassociation()) {
				HandleDisassociation(packet, hdr);
			}
		}
	}
	...
	void ApWifiMac::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		...
		if (hdr->IsData()) {
			...
			if (tag != TwtPacketTag::ERROR()) {
				static std::set<uint64_t> mytags;
				mytags.insert(tag.id);
				const char *sep = "";
				...
				ss << "]";
				NS_LOG_UNCOND("RECV = " << ss.str());
				return;
			}
			HandleDataPacket(packet, hdr);
		} else if (hdr->IsMgt()) {
			HandleManagementPacket(packet, hdr);
		} else {
			NS_LOG_UNCOND("Other frame received by AP, passing to RegularWifiMac.");
			// Invoke the receive handler of our parent class to deal with any
			// other frames. Specifically, this will handle Block Ack-related
			// Management Action frames and TWT frames.
			RegularWifiMac::Receive(packet, hdr);
		}
	}
	uint8_t ApWifiMac::GetLastBeaconSequence() const
	{
		return m_lastBeaconSequence;
	}
