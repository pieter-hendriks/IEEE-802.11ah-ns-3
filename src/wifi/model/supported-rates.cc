...
namespace ns3
{
	...
	SupportedRates &SupportedRates::operator=(const SupportedRates &rates)
	{
	...
	void SupportedRates::AddSupportedRate(uint32_t bs)
	{
	...
	void SupportedRates::SetBasicRate(uint32_t bs)
	{
	...
	bool SupportedRates::IsBasicRate(uint32_t bs) const
	{
	...
	bool SupportedRates::IsSupportedRate(uint32_t bs) const
	{
		uint8_t rate = bs / 500000;
		for (uint8_t i = 0; i < m_nRates; i++) {
	...
	uint8_t SupportedRates::GetNRates(void) const
	{
		return m_nRates;
	}
	...
	uint8_t SupportedRates::GetInformationFieldSize() const
	{
	...
	uint8_t SupportedRates::DeserializeInformationField(Buffer::Iterator start, uint8_t length)
	{
		NS_ASSERT(length <= 8);
		m_nRates = length;
	...
	void ExtendedSupportedRatesIE::SetSupportedRates(SupportedRates *sr)
	{
		m_supportedRates = sr;
	}

	uint8_t ExtendedSupportedRatesIE::GetInformationFieldSize() const
	{
	...
	void ExtendedSupportedRatesIE::SerializeInformationField(Buffer::Iterator start) const
	{
	...
	Buffer::Iterator ExtendedSupportedRatesIE::Serialize(Buffer::Iterator start) const
	{
		// If there are 8 or fewer rates then we don't need an Extended
	...
	uint16_t ExtendedSupportedRatesIE::GetSerializedSize() const
	{
		// If there are 8 or fewer rates then we don't need an Extended
	...
	uint8_t ExtendedSupportedRatesIE::DeserializeInformationField(Buffer::Iterator start, uint8_t length)
	{
		NS_ASSERT(length > 0);
