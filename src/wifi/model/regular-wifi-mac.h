...
namespace ns3
{
	// Define TWT data types, used for managing TWT agreements.
	typedef std::function<bool(const TWTHeader &)> TWTParamaterAcceptanceFunctionType;
	typedef std::function<TWTHeader(const TWTHeader &)> TWTAlternativeConfigurationFunctionType;
	typedef std::function<void()> TWTChangedCallbackFunctionType;
	typedef std::function<void(Mac48Address)> TWTAnnouncementFunctionType;
	bool trivialTwtAccept(const TWTHeader &hdr);

	typedef std::function<void(TWTAgreementKey &, TWTAgreementData &)> TWTStartOfWakePeriodFunctionType;
	typedef std::function<void(TWTAgreementKey &, TWTAgreementData &)> TWTEndOfWakePeriodFunctionType;
	typedef std::function<Time(TWTAgreementKey &, TWTAgreementData &)> TWTComputeNextTwtFunctionType;
	struct PacketData
	{
		Ptr<Packet> packet;
		WifiMacHeader header;
	};
	typedef std::map<Mac48Address, std::queue<PacketData>> TWTPacketQueue;
	...
	public:
		...
		typedef void (*PacketDroppedCallback)(Ptr<const Packet> packet, DropReason reason);
		...
		/**
		 * Functions for handling TWT related frames
		 */
		void HandleTwtFrame(Ptr<Packet> packet, const WifiMacHeader *hdr);

		Time GetNextWakeUpTime() const;
		Time GetNextWakeUpTimeForAddress(const Mac48Address &MacAddress) const;

		/**
		 * Functions setting the TWT configurable parameter functions
		 * Value of getters here is debatable - most likely will never see significant use.
		 * Providing them doesn't hurt either tho, so meh.
		 */
		void SetTWTAcceptanceFunction(TWTParamaterAcceptanceFunctionType);
		TWTParamaterAcceptanceFunctionType GetTWTAcceptanceFunction() const;
		void SetTWTAlternativeConfigurationFunction(TWTAlternativeConfigurationFunctionType);
		TWTAlternativeConfigurationFunctionType GetTWTAlternativeConfigurationFunction() const;
		void SetTWTEndOfWakePeriodCallback(TWTEndOfWakePeriodFunctionType);
		void SetTWTStartOfWakePeriodCallback(TWTStartOfWakePeriodFunctionType);
		TWTEndOfWakePeriodFunctionType GetTWTEndOfWakePeriodCallback() const;
		TWTStartOfWakePeriodFunctionType GetTWTStartOfWakePeriodCallback() const;
		/**
		 * Handle callback stuff, allowing upper layers to control their own behaviours.
		 */
		void SetTWTCallbackFunction(TWTChangedCallbackFunctionType);
		TWTChangedCallbackFunctionType GetTWTCallbackFunction() const;
		void UnsetTWTCallbackFunction();

		// These two exist for debugging purposes, mostly.
		// They are publicly accessible to allow main() to schedule these so we can setup an
		// example scenario.
		void SendTwtSetupFrame(const Mac48Address &destination);
		void SendTwtTeardownFrame(const Mac48Address &destination, uint8_t flowId);

	protected:
		virtual void DoInitialize();
		virtual void DoDispose();
		virtual uint8_t GetLastBeaconSequence() const;
		...
		/**
		 * Functions for internal readability.
		 * More specific handling of TWT frames.
		 */
		void HandleTwtConfirmationFrame(Ptr<Packet> packet, const WifiMacHeader *hdr, TWTHeader *twtHeader);
		void HandleTwtTeardownFrame(Ptr<Packet> packet, const WifiMacHeader *hdr, TWTHeader *twtHeader);
		void HandleTwtSetupFrame(Ptr<Packet> packet, const WifiMacHeader *hdr, TWTHeader *twtHeader);
		void HandleTwtInformationFrame(Ptr<Packet> packet, const WifiMacHeader *hdr, TWTHeader *twtHeader);
		void HandleLocalTwtTeardown(const Mac48Address &destination, uint8_t flowId);
		void SendTwtConfirmationFrame(Ptr<Packet> packet, const WifiMacHeader *sourceHeader, TWTHeader *sourceTwtHeader);
		void SendTwtAlternativeFrame(Ptr<Packet> packet, const WifiMacHeader *sourceHeader, TWTHeader *sourceTwtHeader);
		void SendTwtFrame(TWTHeader &header, Mac48Address dest);
		void CreateLocalTwtAgreement(TWTHeader &header, Mac48Address from);
		void SendTwtAcknowledgmentIfNecessary(Ptr<Packet> packet, const WifiMacHeader *hdr, const TWTHeader &twtHeader);
		void RequestingSTASendTwtAcknowledgment(Ptr<Packet> packet, const WifiMacHeader *hdr, const TWTHeader &twtHeader);
		void RespondingSTASendTwtAcknowledgment(Ptr<Packet> packet, const WifiMacHeader *hdr, const TWTHeader &twtHeader);
		TWTAgreementData *GetTwtAgreementIfExists(const TWTAgreementKey &key);
		std::vector<TWTAgreementData *> GetTwtAgreements(const Mac48Address &addr);
		void HandleNextTwtInfoField(TWTAgreementKey &key, TWTAgreementData &data, std::pair<uint8_t, uint64_t> nextTwt);
		virtual void SendAnnouncedTwtWakeupMessage(Mac48Address to);
		void SendTwtTestTraffic(Mac48Address to);
		void QueueWithTwt(Ptr<Packet> packet, const WifiMacHeader &header);
		void SendQueuedPackets(const Mac48Address &to);
		void HandleTwtAnnouncementFrame(Ptr<Packet> packet, const WifiMacHeader *hdr);
		bool HandleExplicitTwtTimeUpdateIfNeeded(TWTAgreementKey &key, TWTAgreementData &data);
		void HandleTwtTimeUpdateMessage(TWTAgreementKey &key, TWTAgreementData &data);

		/**
		 * Functions handling the start and end of each TWT agreement's wakeups.
		 * These ensure that the correct packets are sent, values are updated etc to allow the protocol to work.
		 */
		void HandleStartOfWakePeriod(TWTAgreementKey &agreementKey, TWTAgreementData &agreement);
		void HandleEndOfWakePeriod(TWTAgreementKey &agreementKey, TWTAgreementData &agreement);
		/**
		 * Function to retrieve the TWT agreements. Optional parameter is pointer to a mac address. If specified (i.e. != nullptr),
		 * We dereference the pointer and only return those agreements with the pointed-to mac address.
		 */
		std::list<const TWTHeader *> GetTwtAgreements(const Mac48Address *address = nullptr) const;

		// Helpers for readable receive function
		void HandleManagementActionFrame(Ptr<Packet> packet, const WifiMacHeader *hdr);
		void HandleControlFrame(Ptr<Packet> packet, const WifiMacHeader *hdr);
		void HandlePsPollFrame(Ptr<Packet> packet, const WifiMacHeader *hdr);
		void HandleTackFrame(Ptr<Packet> packet, const WifiMacHeader *hdr);
	...
	private:
		...
		/**
		 * Members required for TWT management
		 */
		TWTAgreementMap m_twtAgreements;
		TWTPacketQueue m_twtPacketQueue;

		// Functions for accepting/refusing certain TWT agreements, as well as suggesting alternatives as desired.
		// For now, these are not implemented.
		TWTParamaterAcceptanceFunctionType m_twtParameterAcceptanceFunction;
		TWTAlternativeConfigurationFunctionType m_twtAlternativeConfigurationFunction;
		// Callback when anything about the twt configuration changes (updated timings, added entries, ..)
		TWTChangedCallbackFunctionType m_twtChangedCallback;

		// Two main points in time where we need to do anything with TWT.
		// At the start and at the end of each waking period.
		// This class handles the base required functionality.
		// Derived classes can set these members in order to implement additional behaviour.
		TWTStartOfWakePeriodFunctionType m_twtStartOfWakePeriodCallback;
		TWTEndOfWakePeriodFunctionType m_twtEndOfWakePeriodCallback;
		TWTComputeNextTwtFunctionType m_computeNextTwtValue;
	};
