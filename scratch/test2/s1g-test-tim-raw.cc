...
A Page slice element only support one page
 ...
 Between two DTIM beacon, there are many TIM beacons, only allow a TIM beacon include some blocks of one page is called page slice. One TIM
beacon is called a page slice. Page slcie element specify number of page slice between two DTIM, number of blocks in each page slice. Page
slice element only appears together with DTIM.

 Details:
 Page slice element also indicates AP has buffered data for which block, if a station is in that block, the station should first sleep, then
wake up at coresponding page slice(TIM beacon) which includes that block.

 When station wake up at that block, it check whether AP has data for itself. If has, keep awake to receive packets and go to sleep in the
next beacon.
 */
...
void configurePageSlice(void)
{
	...
	// std::cout << "pageIndex=" << (int)config.pageIndex << ", pagePeriod=" << (int)config.pagePeriod << ", pageSliceLength=" <<
	// (int)config.pageSliceLength << ", pageSliceCount=" << (int)config.pageSliceCount << ", blockOffset=" << (int)config.blockOffset << ",
	// timOffset=" << (int)config.timOffset << std::endl;
	// page 0
...
void checkRawAndTimConfiguration(void)
{
	...
	for (uint32_t j = 0; j < config.rps.rpsset.size(); j++) {
		...
		for (uint32_t i = 0; i < config.rps.rpsset[j]->GetNumberOfRawGroups(); i++) {
			totalRawTime += (120 * config.rps.rpsset[j]->GetRawAssigmentObj(i).GetSlotDurationCount() + 500) *
											config.rps.rpsset[j]->GetRawAssigmentObj(i).GetSlotNum();
...
void sendStatistics(bool schedule)
{
	...
	// eventManager.onUpdateSlotStatistics(transmissionsPerTIMGroupAndSlotFromAPSinceLastInterval,
	// 																		transmissionsPerTIMGroupAndSlotFromSTASinceLastInterval);
	// reset
	std::fill(transmissionsPerTIMGroupAndSlotFromAPSinceLastInterval.begin(), transmissionsPerTIMGroupAndSlotFromAPSinceLastInterval.end(),
						0);
	std::fill(transmissionsPerTIMGroupAndSlotFromSTASinceLastInterval.begin(), transmissionsPerTIMGroupAndSlotFromSTASinceLastInterval.end(),
						0);
...
void onSTAAssociated(int i)
{
	...
	for (unsigned k = 0; k < config.rps.rpsset.size(); k++) {
		for (unsigned j = 0; j < config.rps.rpsset[k]->GetNumberOfRawGroups(); j++) {
			if (config.rps.rpsset[k]->GetRawAssigmentObj(j).GetRawGroupAIDStart() <= i + 1 &&
					i + 1 <= config.rps.rpsset[k]->GetRawAssigmentObj(j).GetRawGroupAIDEnd()) {
	...
	if (GetAssocNum() == config.Nsta) {
		cout << "All " << AssocNum << " stations associated at " << Simulator::Now().GetMicroSeconds() << ", configuring clients & server"
				 << endl;
...
void configureNodes(NodeContainer &wifiStaNode, NetDeviceContainer &staDevice)
{
	...
	for (uint32_t i = 0; i < config.Nsta; i++) {
		...
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/$ns3::StaWifiMac/Assoc",
		...
		Config::Connect("/NodeList/" + std::to_string(i) +
										...
										MakeCallback(&NodeEntry::OnNrOfTransmissionsDuringRAWSlotChanged,
																 n)); // not implem

		// Config::Connect("/NodeList/" + std::to_string(i) +
		// "/DeviceList/0/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/$ns3::StaWifiMac/S1gBeaconMissed",
		// MakeCallback(&NodeEntry::OnS1gBeaconMissed, n));
		...
		// hook up TX
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/PhyTxBegin",
										MakeCallback(&NodeEntry::OnPhyTxBegin, n));
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/PhyTxEnd",
										MakeCallback(&NodeEntry::OnPhyTxEnd, n));
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/PhyTxDropWithReason",
										MakeCallback(&NodeEntry::OnPhyTxDrop, n)); //?

		// hook up RX
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/PhyRxBegin",
										MakeCallback(&NodeEntry::OnPhyRxBegin, n));
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/PhyRxEnd",
										MakeCallback(&NodeEntry::OnPhyRxEnd, n));
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/PhyRxDropWithReason",
										MakeCallback(&NodeEntry::OnPhyRxDrop, n));

		// hook up MAC traces
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/RemoteStationManager/MacTxRtsFailed",
										MakeCallback(&NodeEntry::OnMacTxRtsFailed, n)); //?
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/RemoteStationManager/MacTxDataFailed",
										MakeCallback(&NodeEntry::OnMacTxDataFailed, n));
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/RemoteStationManager/MacTxFinalRtsFailed",
										MakeCallback(&NodeEntry::OnMacTxFinalRtsFailed, n)); //?
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/RemoteStationManager/MacTxFinalDataFailed",
										MakeCallback(&NodeEntry::OnMacTxFinalDataFailed, n)); //?

		// hook up PHY State change
		Config::Connect("/NodeList/" + std::to_string(i) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/State/State",
...
{
	if (dataMode == "MCS1_0" || dataMode == "MCS1_1" || dataMode == "MCS1_2" || dataMode == "MCS1_3" || dataMode == "MCS1_4" ||
			dataMode == "MCS1_5" || dataMode == "MCS1_6" || dataMode == "MCS1_7" || dataMode == "MCS1_8" || dataMode == "MCS1_9" ||
			dataMode == "MCS1_10")
		return 1;

	else if (dataMode == "MCS2_0" || dataMode == "MCS2_1" || dataMode == "MCS2_2" || dataMode == "MCS2_3" || dataMode == "MCS2_4" ||
					 dataMode == "MCS2_5" || dataMode == "MCS2_6" || dataMode == "MCS2_7" || dataMode == "MCS2_8")
		return 2;
...
void OnAPPacketToTransmitReceived(string context, Ptr<const Packet> packet, Mac48Address to, bool isScheduled, bool isDuringSlotOfSTA,
																	Time timeLeftInSlot)
{
	int staId = -1;
	if (!config.useV6) {
...
void tcpStateChangeAtServer(TcpSocket::TcpStates_t oldState, TcpSocket::TcpStates_t newState, Address to)
{
	...
	// cout << Simulator::Now().GetMicroSeconds() << " ********** TCP SERVER SOCKET STATE CHANGED FROM " << oldState << " TO " << newState <<
	// endl;
}
	...
	if (nodes[node]->isAssociated) {
		switch (state) {
			case WifiPhy::State::SLEEP: // Sleep
				...
				// NS_LOG_UNCOND(to_string(node + 1) + ",SLEEP," + to_string(start.GetMicroSeconds()) + " " +
...
// MARK FOR MY FUNCTIONS
// TWTFN
// TWT FUNCTIONS
// TWT
template <class T> Ptr<T> GetWifiMac(Ptr<Node> node)
{
	return DynamicCast<T>(DynamicCast<WifiNetDevice>(node->GetDevice(0))->GetMac());
}
...
int main(int argc, char *argv[])
{
	...
	config.NSSFile = config.trafficType + "_" + std::to_string(config.Nsta) + "sta_" + std::to_string(config.NGroup) + "Group_" +
									 std::to_string(config.NRawSlotNum) + "slots_" + std::to_string(config.payloadSize) + "payload_" +
									 std::to_string(config.totaltraffic) + "Mbps_" + std::to_string(config.BeaconInterval) + "BI" + ".nss";
	...
	// Let's not have any propagation loss.
	// Set zero loss!
	channelBuilder.AddPropagationLoss("ns3::LogDistancePropagationLossModel", "Exponent", DoubleValue(3.76), "ReferenceLoss",
																		DoubleValue(0.0), "ReferenceDistance", DoubleValue(1.0));
	...
	mac.SetType("ns3::ApWifiMac", "Ssid", SsidValue(ssid), "BeaconInterval", TimeValue(MicroSeconds(config.BeaconInterval)), "NRawStations",
							UintegerValue(config.NRawSta), "RPSsetup", RPSVectorValue(config.rps), "PageSliceSet", pageSliceValue(config.pageS), "TIMSet",
							TIMValue(config.tim));
	...
	// Config::Set("/NodeList/*/DeviceList/0/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BE_EdcaTxopN/Queue/MaxPacketNumber",
	// 						UintegerValue(10));
	// Config::Set("/NodeList/*/DeviceList/0/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BE_EdcaTxopN/Queue/MaxDelay",
	...
	*/
	...
	// oss << "/NodeList/" << wifiApNode.Get(0)->GetId() << "/DeviceList/0/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/$ns3::ApWifiMac/";
	...
	StaPositionAlloc->Add(Vector(0, 0, 0));
	mobility.SetPositionAllocator(StaPositionAlloc);
	// mobility.SetPositionAllocator("ns3::UniformDiscPositionAllocator", "X", StringValue(std::to_string(xpos)), "Y",
	...
	// 	Config::Connect("/NodeList/" + strSTA + "/DeviceList/0/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/$ns3::StaWifiMac/Assoc",
	// 									MakeCallback(&assoc_record::SetAssoc, m_assocrecord));
	// 	Config::Connect("/NodeList/" + strSTA + "/DeviceList/0/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/$ns3::StaWifiMac/DeAssoc",
	...
	// Config::Connect("/NodeList/" + std::to_string(config.Nsta) + "/DeviceList/0/$ns3::WifiNetDevice/Phy/PhyRxDropWithReason",
	// 								MakeCallback(&OnAPPhyRxDrop));
	// Config::Connect("/NodeList/" + std::to_string(config.Nsta) +
	...
	// Was true - set to false to disable visualizer reporting.
	// This limits console spam so we can see my output.
	sendStatistics(false);
	// TWT Setup
	NS_LOG_UNCOND("Starting setup for TWT test case");

	auto staNode = wifiStaNode.Get(0);
	auto apNode = wifiApNode.Get(0);
	auto staWifiMac = GetWifiMac<StaWifiMac>(staNode);
	auto apWifiMac = GetWifiMac<ApWifiMac>(apNode);
	auto apWifiMacAddress = apWifiMac->GetAddress();

	NS_LOG_UNCOND("TWT Setup completed.");
	NS_LOG_UNCOND("Added TWT setup frame send to simulation.");

	// Schedule TWT setup for halfway through
	Simulator::Schedule(Seconds(config.simulationTime / 2), &RegularWifiMac::SendTwtSetupFrame, GetPointer(staWifiMac), apWifiMacAddress);
	// TWT teardown three quarters of the way through
	Simulator::Schedule(Seconds(3 * config.simulationTime / 4), &RegularWifiMac::SendTwtTeardownFrame, GetPointer(staWifiMac),
											apWifiMacAddress, 0);

	Simulator::Stop(Seconds(config.simulationTime +
													config.CoolDownPeriod)); // allow up to a minute after the client & server apps are finished to process the queue
	NS_LOG_UNCOND("Starting simulation. TWT request send is scheduled for halfway through simulation.");
	Simulator::Run();
	...
	// 	cout << i << " sent: " << stats.get(i).NumberOfSentPackets << " ; delivered: " << stats.get(i).NumberOfSuccessfulPackets
	// 			 << " ; echoed: " << stats.get(i).NumberOfSuccessfulRoundtripPackets
	// 			 << "; packetloss: " << stats.get(i).GetPacketLoss(config.trafficType) << endl;
	// }
	...
	// risultati << "Sta "
	// 						 "node#,distance,timerx(notassociated),timeidle(notassociated),timetx(notassociated),timesleep(notassociated),timecollision("
	// 						 "notassociated)"
	// 					<< std::endl;
	// unsigned i = 0;
	// string spazio = ",";
	...
	// 	risultati << i << spazio << dist[i] << spazio << timeRxArray[i].GetSeconds() << ",(" << timeRxNotAssociated[i].GetSeconds() << "),"
	// 						<< timeIdleArray[i].GetSeconds() << ",(" << timeIdleNotAssociated[i].GetSeconds() << ")," << timeTxArray[i].GetSeconds()
	// 						<< ",(" << timeTxNotAssociated[i].GetSeconds() << ")," << timeSleepArray[i].GetSeconds() << ",("
	// 						<< timeSleepNotAssociated[i].GetSeconds() << ")," << timeCollisionArray[i].GetSeconds() << ",("
